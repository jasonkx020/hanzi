		...
		<view class="lab-map">
			...
			<view class="lab-map-actions">
				<view class="lab-map-action" @click="goChart">
					...
					<text class="lab-map-action-text">韵母表</text>
...
import { VOWEL_LEVELS } from '@/utils/pinyin-vowel-lab/constants.js'
import { loadVowelLabProgress, isVowelLevelUnlocked } from '@/utils/pinyin-vowel-lab/progress.js'

const ROUTES = {
	1: '/pages/pinyin/vowel-lab/ear',
	2: '/pages/pinyin/vowel-lab/mouth',
	3: '/pages/pinyin/vowel-lab/group'
}
...
export default {
	...
	data() {
		return {
			levels: VOWEL_LEVELS,
			progress: loadVowelLabProgress()
		}
	},
	onShow() {
		this.progress = loadVowelLabProgress()
	},
	methods: {
		nodeUnlocked(id) {
			if (id === 1) return true
			return isVowelLevelUnlocked(id, this.progress)
		},
		nodeDone(id) {
		...
		onTapLevel(lv) {
			if (lv.isLink) {
				...
				uni.navigateTo({ url: '/pages/pinyin/drill?category=vowel' })
				return
			}
			if (!this.nodeUnlocked(lv.id)) {
				const hint =
		...
		goChart() {
			uni.navigateTo({ url: '/pages/pinyin/vowel-lab/chart' })
		}
	}
}
</script>
...
.lab-map-action {
	padding: 24rpx;
	border-radius: 24rpx;
	background: linear-gradient(135deg, #f5e8ff 0%, #fff8f0 100%);
