...
export default {
	...
	methods: {
		...
		finishQuiz() {
			const pass = this.correctCount >= VOWEL_EAR_QUIZ_PASS
			const p = loadVowelLabProgress()
			saveVowelLabProgress({ earBest: Math.max(Number(p.earBest) || 0, this.correctCount) })
			if (pass) {
				awardVowelLevelStar(1, this.correctCount >= 7 ? 3 : 2)
				uni.showModal({
					title: '过关啦',
					content: '小耳朵真灵！下一步「发音小贴士」',
					showCancel: false,
				...
				uni.showModal({
					title: '再试一次',
					content: `对了 ${this.correctCount} 题，需要 ${VOWEL_EAR_QUIZ_PASS} 题过关`,
					confirmText: '再来',
					cancelText: '返回',
					success: (res) => {
						if (res.confirm) {
							this.qIndex = 0
							this.correctCount = 0
							this.questions = buildVowelEarQuestions(VOWEL_EAR_QUIZ_TOTAL)
							this.$nextTick(() => this.playCurrent())
...
.quiz-play-zone {
	...
	padding: 20rpx;
	border-radius: 24rpx;
	background: linear-gradient(135deg, #f5e8ff, #fff5fa);
}
.quiz-hint {
	font-size: 28rpx;
	font-weight: 700;
