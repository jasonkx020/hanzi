<template>
	<meng-sub-page title="同组小能手" subtitle="同一组声母，听音辨认" avatar-pose="happy" :overlap-body="true">
		<view class="quiz-page">
			...
			<view v-if="currentQ?.section" class="group-badge">
				...
				<text class="group-badge-sub">这一组里的声母，仔细听</text>
			</view>
			...
			<view class="letter-grid letter-grid--group">
				<view
...
import { INITIAL_GROUP_QUIZ_PASS } from '@/utils/pinyin-initial-lab/constants.js'
import { buildInitialGroupQuestions } from '@/utils/pinyin-initial-lab/quiz.js'
import {
...
export default {
	...
	data() {
		return {
			questions: buildInitialGroupQuestions(),
			qIndex: 0,
			correctCount: 0,
			pickedIndex: -1,
			playing: false,
	...
	methods: {
		...
		finishQuiz() {
			const pass = this.correctCount >= INITIAL_GROUP_QUIZ_PASS
			const p = loadInitialLabProgress()
			saveInitialLabProgress({ groupBest: Math.max(Number(p.groupBest) || 0, this.correctCount) })
			if (pass) {
				awardInitialLevelStar(3, this.correctCount >= 6 ? 3 : 2)
				uni.showModal({
					title: '过关啦',
					content: '同组声母难不倒你！可以去「大闯关」',
					showCancel: false,
				...
				uni.showModal({
					title: '再试一次',
					content: `对了 ${this.correctCount} 题，需要 ${INITIAL_GROUP_QUIZ_PASS} 题过关`,
					confirmText: '再来',
					cancelText: '返回',
					success: (res) => {
						if (res.confirm) {
							this.qIndex = 0
							this.correctCount = 0
							this.questions = buildInitialGroupQuestions()
							this.$nextTick(() => this.playCurrent())
