...
-->
<template>
	<meng-sub-page title="声母乐园" subtitle="听一听，声母就会啦" avatar-pose="happy" :overlap-body="true">
		<view class="lab-map">
			...
			<view class="lab-map-actions">
				<view class="lab-map-action" @click="goChart">
					...
					<text class="lab-map-action-text">声母表</text>
...
import { INITIAL_LEVELS } from '@/utils/pinyin-initial-lab/constants.js'
import { loadInitialLabProgress, isInitialLevelUnlocked } from '@/utils/pinyin-initial-lab/progress.js'

const ROUTES = {
	1: '/pages/pinyin/initial-lab/ear',
	2: '/pages/pinyin/initial-lab/mouth',
	3: '/pages/pinyin/initial-lab/group'
}
...
export default {
	...
	data() {
		return {
			...
			progress: loadInitialLabProgress()
		}
	},
	onShow() {
		this.progress = loadInitialLabProgress()
	},
	methods: {
		nodeUnlocked(id) {
			if (id === 1) return true
			return isInitialLevelUnlocked(id, this.progress)
		},
		nodeDone(id) {
		...
		onTapLevel(lv) {
			if (lv.isLink) {
				...
				uni.navigateTo({ url: '/pages/pinyin/drill?category=initial' })
				return
			}
			if (!this.nodeUnlocked(lv.id)) {
				const hint =
		...
		goChart() {
			uni.navigateTo({ url: '/pages/pinyin/initial-lab/chart' })
		}
	}
}
</script>
